...
const middleware = (req, res, next) => {
  ...
  //decrypt the header
  const { signature, payloadSecretKey, merchantId, metaData, base, apolloKey } = config;
  const decipher = crypto.createDecipheriv(
    signature,
    Buffer.from(payloadSecretKey),
    merchantId
  );
  ...
  if (
    ...
    //@ts-ignore
    let _decryptedData = decipher.update(encryptedData, base, metaData);
    //@ts-ignore
    _decryptedData += decipher.final(metaData);
    const { apolloAccessKey } = JSON.parse(_decryptedData);

    if(apolloAccessKey !== apolloKey) throw new Error("Invalid Request by apollo");
  } else if (encryptedData && req.body?.query) {
    //@ts-ignore
  ...
  app.use((err, req, res, next) => {
    ...
    if (isCelebrateError(err)) {
      ...
      LoggerInstance.error(
        "JOI Validating API: %s, error: %s, body: %o",
        req.originalUrl,
        message,
        req.body
      );
